...
ssh: ## ssh into host
	@ssh "${HOMELAB_USER}@${HOMELAB_HOSTNAME}"

ssh-local: ## ssh into host
	@ssh "${HOMELAB_USER}@${HOMELAB_IP}"

deploy: ## deploy playbook
	@ansible-playbook -i ansible/inventory.py ansible/playbook.yml --extra-vars "state=present"

teardown: ## teardown playbook
	@ansible-playbook -i ansible/inventory.py ansible/playbook.yml --extra-vars "state=absent"
